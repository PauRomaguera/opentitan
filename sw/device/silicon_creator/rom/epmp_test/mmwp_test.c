...
/*
bazel test --test_output=streamed --test_timeout=999999 --copt=-DDEBUG --copt=-DDISABLE_EFLASH_RULE disk_cache=~/bazel_cache //sw/device/silicon_creator/rom:mmwp_test
...
#endif
//simple print MMWP and eFLASH configured or not
static void log_mmwp_and_build_flags(void) {
  uint32_t mseccfg;
  CSR_READ(CSR_REG_MSECCFG, &mseccfg);
  unsigned mmwp = (mseccfg & EPMP_MSECCFG_MMWP) ? 1u : 0u;

#ifdef DISABLE_EFLASH_RULE
  const char *disable_eflash_rule = "1";
#else
  const char *disable_eflash_rule = "0";
#endif

  LOG_INFO("EPMP status: MMWP=%u, DISABLE_EFLASH_RULE=%s", mmwp, disable_eflash_rule);
}
...
void rom_main(void) {
  ...
  LOG_INFO("1) Testing eFlash read permission");
  test_read_eflash();
  
  LOG_INFO("2) Testing eFlash write protection");
  test_nowrite_eflash();

  LOG_INFO("3) Testing eFlash execution protection");
  test_noexec_eflash();   
